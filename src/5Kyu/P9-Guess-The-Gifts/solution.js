...
function guessGifts(wishlist, presents) {
    ...
    for(let i = 0; i < wishlist.length; i++){
        for(let [key, value] of Object.entries(presents)){
            if(wishlist[i].size === value.size && wishlist[i].clatters === value.clatters  && wishlist[i].weight === value.weight ){
              if(!array.includes(wishlist[i].name)) array.push(wishlist[i].name)
            } 
        }
    }
